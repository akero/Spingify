		  ...
		  public void done(ParseException e) {
		    if (e == null) {
		      ...
		      try{
					...
					fos = getApplicationContext().openFileOutput("isloggedin.txt", Context.MODE_APPEND);
					
						fos.write("yes".getBytes());
					fos.close();
				}catch(Exception ef){
					Log.d("error is "+ef.toString(),"gotcha123");
					//Toast.makeText(Login2.this, "Failed, try emptying space in internal storage",  Toast.LENGTH_SHORT ).show();
				}
		      Intent in = new Intent(getApplicationContext(),org.yaaic.activity.ServersActivity.class);
		      startActivity(in);
		    } else {
